...
const MATERIALS_OPTIONS = [
  "Premium fixtures and hardware", "Custom cabinetry", "Hardwood flooring",
  "Tile and stone", "High-end countertops (quartz/granite)", "Energy-efficient windows",
  ...
  return (
    <>
      ...
      <form name="estimator-lead" data-netlify="true" className="hidden">
        ...
        <input name="projectType" /><input name="complexity" /><input name="sqft" /><input name="estimatedMid" />
      </form>
      ...
      <main className="pt-[68px] min-h-screen">
        <div className="container py-16 sm:py-24 max-w-3xl">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            ...
            <h1 className="text-4xl sm:text-5xl font-semibold mb-4" style={{ fontFamily: "var(--font-heading)" }}>
              What will your<br /><em className="text-primary italic">project cost?</em>
            </h1>
            <p className="text-muted-foreground font-light text-lg">
              Get a real estimate for Eugene, OR construction — powered by local market data.
            </p>
          </motion.div>
          ...
          {step < 4 && (
            <div className="flex gap-2 mb-8">
              {[1,2,3].map(s => (
                <div key={s} className={`h-1 flex-1 rounded-full transition-colors duration-300 ${step >= s ? "bg-primary" : "bg-border/60"}`} />
              ))}
            </div>
          )}
          ...
          {step === 2 && (
            <div>
              <h2 className="text-xl font-semibold mb-5" style={{ fontFamily: "var(--font-heading)" }}>Project details</h2>
              <div className="space-y-5 mb-6">
                <div>
                  <label className="block text-[10px] tracking-[0.2em] uppercase text-muted-foreground/70 mb-2 font-medium"
                         style={{ fontFamily: "var(--font-condensed)" }}>
                    Square Footage (approximate)
                  </label>
                  <input type="number" value={sqft} onChange={e => setSqft(e.target.value)}
                         placeholder="e.g. 1200" min="50" max="10000"
                ...
                <div>
                  <label className="block text-[10px] tracking-[0.2em] uppercase text-muted-foreground/70 mb-3 font-medium"
                         style={{ fontFamily: "var(--font-condensed)" }}>
                    Finish Level
                  </label>
                    ...
                    {(["low", "medium", "high"] as const).map(c => (
                      <button key={c} onClick={() => setComplexity(c)}
                              className={`py-3 text-center border text-sm font-medium transition-all ${
                                complexity === c ? "border-primary bg-primary/10 text-foreground" : "border-border/60 text-muted-foreground hover:border-primary/40"
                              }`}>
                        {c === "low" ? "Builder Grade" : c === "medium" ? "Mid-Range" : "Premium"}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex gap-3">
                <button onClick={() => setStep(1)}
                        ...
                        style={{ fontFamily: "var(--font-condensed)" }}>← Back</button>
                <button onClick={() => setStep(3)}
                        ...
                        style={{ fontFamily: "var(--font-condensed)" }}>Next: Materials →</button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div>
              <h2 className="text-xl font-semibold mb-2" style={{ fontFamily: "var(--font-heading)" }}>Material preferences</h2>
              <p className="text-sm text-muted-foreground font-light mb-5">Select any that apply (optional)</p>
                ...
                {MATERIALS_OPTIONS.map(m => (
                  <button key={m} onClick={() => toggleMaterial(m)}
                          className={`p-3 border text-left text-sm transition-all flex items-center gap-3 ${
                            materials.includes(m) ? "border-primary bg-primary/10" : "border-border/60 text-muted-foreground hover:border-primary/40"
                          }`}>
                    <div className={`h-4 w-4 border flex-shrink-0 flex items-center justify-center ${
                      materials.includes(m) ? "border-primary bg-primary" : "border-border/60"
                    }`}>
                      {materials.includes(m) && <CheckCircle2 className="h-3 w-3 text-primary-foreground" />}
                    </div>
                    {m}
                  </button>
                ))}
              ...
              <div className="flex gap-3">
                ...
                <button onClick={runEstimate} disabled={loading}
                        ...
                        style={{ fontFamily: "var(--font-condensed)" }}>
                  {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Estimating…</> : <>Get Estimate →</>}
                </button>
              </div>
            </div>
          )}
            ...
            <div className="space-y-5">
              ...
              <div className="bg-card border border-border/60 p-6">
                <p className="text-[10px] font-bold tracking-[0.18em] uppercase text-muted-foreground mb-4"
                   style={{ fontFamily: "var(--font-condensed)" }}>Estimated Project Cost</p>
                <div className="grid grid-cols-3 gap-4 mb-5">
                  {[
                    { label: "Conservative", value: result.estimatedLow, cls: "text-muted-foreground" },
                    { label: "Expected",     value: result.estimatedMid, cls: "text-primary text-3xl!" },
                    { label: "Premium",      value: result.estimatedHigh, cls: "text-muted-foreground" },
                  ].map(({ label, value, cls }) => (
                    <div key={label} className="text-center">
                      <p className={`text-xl sm:text-2xl font-bold mb-1 ${cls}`}>{fmt(value)}</p>
              ...
              <div className="bg-card border border-border/60 p-5">
                <p className="text-[10px] font-bold tracking-[0.18em] uppercase text-muted-foreground mb-2"
                   style={{ fontFamily: "var(--font-condensed)" }}>Estimate Basis</p>
                <p className="text-sm text-muted-foreground font-light leading-relaxed">{result.aiReasoning}</p>
                <p className="text-[10px] text-muted-foreground/50 mt-3 font-light">
                  Based on current Eugene, OR market data. Actual costs may vary. Free on-site estimate available.
                </p>
              </div>
              ...
              {!leadSent ? (
                <div className="bg-card border border-border/60 p-6">
                  <p className="text-[10px] font-bold tracking-[0.18em] uppercase text-muted-foreground mb-2"
                     style={{ fontFamily: "var(--font-condensed)" }}>Get a Detailed Quote</p>
                  <p className="text-sm text-muted-foreground font-light mb-4">
                    Ready for a real, on-site estimate? Eric will come to you — free.
                  </p>
                  <div className="space-y-3">
                    <input value={leadName} onChange={e => setLeadName(e.target.value)} placeholder="Your name *"
                           className="w-full px-4 py-3 bg-input border border-border text-foreground text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary/60" />
                    <div className="grid sm:grid-cols-2 gap-3">
                      <input value={leadEmail} onChange={e => setLeadEmail(e.target.value)} placeholder="Email *" type="email"
                             className="w-full px-4 py-3 bg-input border border-border text-foreground text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary/60" />
                      <input value={leadPhone} onChange={e => setLeadPhone(e.target.value)} placeholder="Phone" type="tel"
                             className="w-full px-4 py-3 bg-input border border-border text-foreground text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary/60" />
                    </div>
                    <button onClick={submitLead} disabled={!leadName || !leadEmail}
                            className="w-full py-3 bg-primary text-primary-foreground text-[11px] font-bold tracking-widest uppercase hover:bg-primary/85 disabled:opacity-50 transition-colors"
                            style={{ fontFamily: "var(--font-condensed)" }}>
                      Request Free On-Site Estimate <ArrowRight className="inline h-3.5 w-3.5 ml-1" />
                    </button>
                    <p className="text-[10px] text-center text-muted-foreground/50">No obligation · {SITE.license}</p>
                  </div>
                </div>
              ) : (
                <div className="bg-card border border-border/60 p-8 text-center">
                  <CheckCircle2 className="h-10 w-10 text-green-400 mx-auto mb-3" />
                  <p className="font-semibold mb-1" style={{ fontFamily: "var(--font-heading)" }}>Request received.</p>
                  <p className="text-sm text-muted-foreground font-light">Eric will be in touch within one business day.</p>
                  <a href={SITE.phoneHref} className="block mt-4 text-primary text-sm hover:underline">{SITE.phone}</a>
                </div>
              )}
            </div>
          )}
