...
import {
  Eye, EyeOff, Globe, Image, Loader2, Plus, Star, Trash2, X,
} from "lucide-react";
...
const BLANK_FORM = {
  ...
  location: "", completionYear: new Date().getFullYear(), squareFootage: "",
  coverImageUrl: "", clientTestimonial: "", clientName: "",
  featured: false, published: false, sortOrder: 0,
};

const CATEGORIES = [
  "Custom Home", "Remodel", "Addition", "New Construction",
  "Restoration", "Outdoor Living", "Roofing", "Cabinets & Millwork", "Painting",
];
...
export default function PortfolioAdmin() {
  ...
  const f = (key: keyof typeof BLANK_FORM) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground font-light mt-0.5">
              Manage public portfolio — publish, feature, and add project showcases
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-3 gap-4 mb-5">
          {[
            { label: "Total", value: projects?.length ?? 0, color: "text-foreground" },
            { label: "Published", value: projects?.filter(p => p.published).length ?? 0, color: "text-green-400" },
            { label: "Featured", value: projects?.filter(p => p.featured).length ?? 0, color: "text-primary" },
          ].map(s => (
            <div key={s.label} className="bg-card border border-border/60 p-4 text-center">
        ...
        {showForm && (
          <div className="bg-card border border-primary/30 p-5 mb-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              <input value={form.location} onChange={f("location")} placeholder="Location (Eugene, OR)"
              ...
              <input value={form.completionYear} onChange={f("completionYear")} type="number" placeholder="Year completed"
                     className="px-3 py-2 bg-input border border-border text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary/60" />

              <input value={form.squareFootage} onChange={f("squareFootage")} type="number" placeholder="Square footage"
                     className="px-3 py-2 bg-input border border-border text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary/60" />

              <input value={form.coverImageUrl} onChange={f("coverImageUrl")} placeholder="Cover image URL"
              ...
              <textarea value={form.shortDescription} onChange={f("shortDescription")}
                        placeholder="Short description (shown in portfolio grid, max 500 chars)" rows={2}
              ...
              <input value={form.clientName} onChange={f("clientName")} placeholder="Client name (for testimonial)"
                     className="px-3 py-2 bg-input border border-border text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary/60" />

              <textarea value={form.clientTestimonial} onChange={f("clientTestimonial")}
                        placeholder="Client testimonial quote" rows={2}
            ...
            <div className="flex gap-4 mt-4">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-xs text-foreground">Publish to public site</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="accent-primary"
                />
                <span className="text-xs text-foreground">Featured project</span>
              </label>
            </div>
        ...
        {!isLoading && (!projects || projects.length === 0) && (
          <div className="py-20 text-center">
            ...
            <p className="text-sm text-muted-foreground font-light mb-1">No portfolio projects yet</p>
          ...
          {projects?.map(p => (
            <div key={p.id}
              ...
              <div className="w-20 h-16 shrink-0 border border-border/40 overflow-hidden bg-muted/20">
                {p.cover_image_url ? (
                  <img src={p.cover_image_url} alt={p.title}
                       className="w-full h-full object-cover"
                       onError={e => { (e.target as HTMLImageElement).parentElement!.innerHTML = '<div class="w-full h-full flex items-center justify-center"><span class="text-muted-foreground/30">No img</span></div>'; }} />
                ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                {p.short_description && (
                  <p className="text-xs text-muted-foreground/70 mt-1 line-clamp-1">{p.short_description}</p>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  style={{ fontFamily: "var(--font-condensed)" }}
                >
                  {p.published ? <Globe className="h-2.5 w-2.5" /> : <EyeOff className="h-2.5 w-2.5" />}
