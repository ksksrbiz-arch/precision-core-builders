...
import {
  ...
  ExternalLink, Key, Loader2, RefreshCw, Shield, X,
  CreditCard, CloudRain, Webhook, Map,
} from "lucide-react";
...
const SERVICES: ServiceKey[] = [
  {
    ...
    icon: Shield,
    description: "Powers field report generation, lead scoring, AI chat, and estimating.",
    optional: false,
    configured: true,
    guideSteps: [],
    guideUrl: "",
  ...
  },
  {
    ...
    icon: CreditCard,
    description: "Sends milestone invoices and creates client payment links. 100% optional.",
    optional: true,
    configured: null,
    guideSteps: [
  ...
  },
  {
    ...
    icon: Webhook,
    description: "Sends SMS/email alerts when field reports are submitted or materials run short.",
    optional: true,
    configured: null,
    guideSteps: [
  ...
  },
  {
    ...
    icon: Map,
    description: "Interactive job site maps on the Contact page and Project detail view.",
    optional: true,
    configured: null,
    guideSteps: [
...
function ServiceCard({ svc, adminToken }: { svc: ServiceKey; adminToken: string }) {
  ...
  const save = async () => {
    if (!value.trim()) { toast.error("Paste your key first"); return; }
    ...
    try {
      ...
      setOpen(false);
      setValue("");
      toast.success(`${svc.label} configured! Redeploy triggered -- live in ~60s.`);
    } catch (err) {
  ...
  return (
    <div className={`bg-card border transition-colors ${
      ...
      <div
        ...
        onClick={() => !isConfigured && svc.guideSteps.length > 0 && setOpen(v => !v)}
      >
        <div className={`h-9 w-9 border flex items-center justify-center shrink-0 ${
          isConfigured ? "border-green-400/40 bg-green-400/10" : "border-border/60"
        }`}>
          {isConfigured
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {svc.optional && (
              <span className="text-[9px] px-1.5 py-0.5 border border-border/40 text-muted-foreground/60 tracking-widest uppercase"
                    style={{ fontFamily: "var(--font-condensed)" }}>Optional</span>
            )}
          </div>
          <p className="text-xs text-muted-foreground font-light mt-0.5">{svc.description}</p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {isConfigured ? (
            ...
            <span className="text-[10px] text-amber-400 font-bold tracking-widest uppercase"
                  style={{ fontFamily: "var(--font-condensed)" }}>Not set</span>
          )}
          {!isConfigured && svc.guideSteps.length > 0 && (
      ...
      {open && !isConfigured && (
        <div className="px-5 pb-5 border-t border-border/40">
            ...
            <ol className="space-y-3">
              {svc.guideSteps.map((step, i) => (
                <li key={i} className="flex gap-3">
                  <span className="h-5 w-5 rounded-full border border-primary/40 bg-primary/10 text-primary text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5"
                        style={{ fontFamily: "var(--font-condensed)" }}>
                    {i + 1}
                  </span>
          ...
          {/* External link */}
          {svc.guideUrl && (
            <a href={svc.guideUrl} target="_blank" rel="noopener noreferrer"
          ...
          <div className="bg-background/60 border border-border/60 p-4">
            ...
            <p className="text-[10px] text-muted-foreground/50 mt-2">
              Your key is sent directly to Netlify's encrypted environment -- never stored in the app.
              A redeploy triggers automatically so it goes live in about 60 seconds.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-muted-foreground font-light">
            One-time configuration for your Digital Foreman platform.
            Expand any service to see a plain-English setup guide.
          </p>
        </div>
        ...
        <div className="bg-card border border-border/60 p-5 mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm font-bold text-primary">{configured} / {total}</span>
          </div>
        ...
        <div className="border border-primary/20 bg-primary/5 p-4 mb-6 flex gap-3">
          ...
          <div className="text-xs text-muted-foreground leading-relaxed">
            <span className="font-semibold text-foreground">You're in full control.</span>{" "}
            Every service below is your own account -- Precision Core Builders manages no payments,
            stores no keys, and has no access to your Stripe or billing data.
            Keys are saved encrypted directly to your Netlify project.
          </div>
        </div>
        ...
        <div className="mt-8 p-4 border border-border/40 bg-muted/10">
          <p className="text-[10px] text-muted-foreground/60 leading-relaxed">
            <span className="font-semibold text-muted-foreground">After saving any key</span> -- Netlify
            automatically rebuilds and redeploys your site (~60 seconds). You'll see the feature become
            active immediately after. If you need to update a key later, just come back to this page
