...
export default function FieldReportNew() {
  ...
  const parseJSON = (s: string | null): string[] => { try { return JSON.parse(s ?? "[]"); } catch { return []; } };
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {step === "select" && (
          <div className="bg-card border border-border/60 p-6">
            <p className="text-sm text-muted-foreground mb-4 font-light">Select the project for this field report:</p>
            <div className="space-y-2 mb-5">
              {projects?.data.filter(p => p.status === "in_progress" || p.status === "contracted").map(p => (
                <button key={p.id} onClick={() => setProjectId(p.id)}
                        className={`w-full text-left p-4 border transition-colors ${
                          projectId === p.id ? "border-primary bg-primary/5 text-foreground" : "border-border/60 text-muted-foreground hover:border-primary/40"
                        }`}>
            ...
            <button onClick={() => projectId && setStep("record")} disabled={!projectId}
        ...
        {step === "record" && (
          <div className="bg-card border border-border/60 p-8 text-center">
            <p className="text-xs text-muted-foreground mb-8 font-light">
              Project: <strong className="text-foreground">{projects?.data.find(p => p.id === projectId)?.name}</strong>
            </p>
            {/* Recording indicator */}
            <div className={`h-28 w-28 rounded-full border-4 flex items-center justify-center mx-auto mb-6 transition-all ${
              recording ? "border-red-500 bg-red-500/10 animate-pulse" : "border-border/60 bg-card"
            }`}>
              {recording ? <Mic className="h-10 w-10 text-red-400" /> : <Mic className="h-10 w-10 text-muted-foreground" />}
            </div>
            {recording && (
              <p className="text-3xl font-mono text-foreground mb-2">{fmt(recordingTime)}</p>
            )}
            <p className="text-sm text-muted-foreground mb-8 font-light">
              {recording ? "Recording… speak clearly about today's progress, materials, and any issues." : "Press record when ready to report on today's site work."}
            </p>
            ...
            <div className="flex gap-3 justify-center">
              ...
              {audioBlob && !recording && (
                <>
                  <button onClick={() => { setAudioBlob(null); setRecordingTime(0); }}
        ...
        {step === "processing" && (
          <div className="bg-card border border-border/60 p-12 text-center">
            ...
            <p className="text-xs text-muted-foreground font-light">Generating structured field report with AI</p>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="bg-card border border-border/60 p-5">
              <p className="text-[10px] font-bold tracking-[0.18em] uppercase text-muted-foreground mb-3"
                 style={{ fontFamily: "var(--font-condensed)" }}>Report Summary</p>
            ...
            <div className="flex gap-3">
              <button onClick={() => setLocation(`/admin/field-reports/${report.id}`)}
              ...
              <button onClick={publishReport} disabled={publishMutation.isPending}
                      ...
                      style={{ fontFamily: "var(--font-condensed)" }}>
                {publishMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <><CheckCircle2 className="h-4 w-4" /> Publish to Client</>}
              </button>
            </div>
          </div>
        )}
        ...
        {step === "done" && (
          <div className="bg-card border border-border/60 p-10 text-center">
            ...
            <h2 className="text-xl font-semibold mb-2" style={{ fontFamily: "var(--font-heading)" }}>Report Published</h2>
            <p className="text-sm text-muted-foreground font-light mb-6">The client can now view this update in their portal.</p>
            <div className="flex gap-3 justify-center">
              <button onClick={() => setLocation("/admin/field-reports")}
              ...
              <button onClick={() => { setStep("select"); setAudioBlob(null); setReport(null); setRecordingTime(0); }}
