...
import { AlertTriangle, ArrowUpDown, CheckCircle, ClipboardCheck, DollarSign, FileText, Landmark, Milestone, Plus, Shield, StickyNote } from "lucide-react";
...
export default function LedgerView() {
  ...
  const [form, setForm] = useState({ entryType: "note" as string, title: "", description: "", amountDelta: "", visibleToClient: true });
  ...
  const appendMut = trpc.ledger.append.useMutation({
    onSuccess: () => { utils.ledger.list.invalidate(); setShowNew(false); setForm({ entryType: "note", title: "", description: "", amountDelta: "", visibleToClient: true }); toast.success("Ledger entry recorded"); },
  });
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-semibold" style={{ fontFamily: "var(--font-heading)" }}>Core Values Ledger</h1>
        ...
        <p className="text-xs text-muted-foreground font-light mb-5">
          Every decision, permit, inspection, and cost change — permanently recorded. Entries cannot be edited or deleted.
        </p>

        {/* Project selector */}
        <select value={projectId ?? ""} onChange={e => { setProjectId(e.target.value ? Number(e.target.value) : undefined); setPage(1); }}
        ...
        {showNew && projectId && (
          <div className="bg-card border border-primary/30 p-6 mb-6 space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] font-bold tracking-[0.12em] uppercase text-muted-foreground mb-1 block" style={{ fontFamily: "var(--font-condensed)" }}>Type</label>
                <select value={form.entryType} onChange={e => setForm(prev => ({ ...prev, entryType: e.target.value }))}
              ...
              <div>
                <label className="text-[10px] font-bold tracking-[0.12em] uppercase text-muted-foreground mb-1 block" style={{ fontFamily: "var(--font-condensed)" }}>Title *</label>
                <input type="text" value={form.title} onChange={e => setForm(prev => ({ ...prev, title: e.target.value }))}
                  className="w-full bg-input border border-border text-sm text-foreground p-2.5 focus:outline-none focus:border-primary/60" />
              </div>
            </div>
            <div>
              <label className="text-[10px] font-bold tracking-[0.12em] uppercase text-muted-foreground mb-1 block" style={{ fontFamily: "var(--font-condensed)" }}>Description *</label>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] font-bold tracking-[0.12em] uppercase text-muted-foreground mb-1 block" style={{ fontFamily: "var(--font-condensed)" }}>Cost Impact ($)</label>
                <input type="number" value={form.amountDelta} onChange={e => setForm(prev => ({ ...prev, amountDelta: e.target.value }))} placeholder="+500 or -200"
                  className="w-full bg-input border border-border text-sm text-foreground p-2.5 focus:outline-none focus:border-primary/60" />
              </div>
              <div className="flex items-end pb-1">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.visibleToClient} onChange={e => setForm(prev => ({ ...prev, visibleToClient: e.target.checked }))}
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => appendMut.mutate({
                  projectId: projectId!, entryType: form.entryType as any, title: form.title, description: form.description,
                  amountDelta: form.amountDelta ? Number(form.amountDelta) : undefined, visibleToClient: form.visibleToClient,
                })} disabled={!form.title || !form.description || appendMut.isPending}
                ...
                style={{ fontFamily: "var(--font-condensed)" }}>{appendMut.isPending ? "Recording…" : "Record Entry"}</button>
            </div>
          </div>
        )}
        ...
        {!projectId ? (
          <div className="bg-card border border-border/60 p-12 text-center">
            ...
            <p className="text-muted-foreground text-sm">Select a project to view its decision ledger.</p>
          </div>
        ) : isLoading ? (
          ...
          <div className="bg-card border border-border/60 p-12 text-center">
            ...
            <p className="text-muted-foreground text-sm mb-3">No ledger entries yet</p>
            <button onClick={() => setShowNew(true)} className="text-primary text-sm underline">Add the first entry</button>
          </div>
        ) : (
                    ...
                    <div className="bg-card border border-border/60 p-4">
                      <div className="flex items-start justify-between gap-3 mb-1.5">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          {delta && (
                            <span className={`text-xs font-bold ${entry.amount_delta > 0 ? "text-red-400" : "text-green-400"}`}>{delta}</span>
                          )}
                          {entry.visible_to_client && (
                            <span className="text-[9px] text-muted-foreground/40" title="Visible to client">
