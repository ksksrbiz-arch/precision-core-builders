...
export function ServicePage(p: ServicePageProps) {
  ...
  const input = "w-full px-4 py-3 bg-input border border-border text-foreground text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/40 transition-colors";
          ...
          <div className="container relative z-10">
            <motion.div initial="hidden" animate="visible" variants={stagger}>
              <motion.div variants={fadeUp} className="flex items-center gap-3 mb-4">
                ...
                <span className="text-primary text-[11px] tracking-[0.28em] uppercase font-semibold"
            ...
            <div className="grid lg:grid-cols-[1fr_360px] gap-14 xl:gap-20">
              ...
              <div>
                ...
                <div className="space-y-5 mb-10">
                  {p.body.map((para, i) => (
                    <p key={i} className="text-muted-foreground leading-relaxed font-light text-base">{para}</p>
                  ))}
                </div>
                ...
                <ul className="grid sm:grid-cols-2 gap-3 mb-12">
                  {p.includes.map(item => (
                    <li key={item} className="flex items-start gap-3 bg-card border border-border/60 p-4">
                      <CheckCircle2 className="h-4 w-4 text-primary mt-0.5 flex-shrink-0" aria-hidden />
                ...
                <p className="text-muted-foreground font-light mb-3">
                  ...
                  <strong className="text-foreground font-medium">{p.title.toLowerCase()}</strong>{" "}
                  services throughout:
                </p>
                ...
                <div className="space-y-4 mb-12">
                  {p.faqs.map(({ q, a }) => (
                    <details key={q} className="group border border-border/60 bg-card">
                      <summary className="flex items-center justify-between gap-4 p-5 cursor-pointer list-none min-h-[56px]">
                        <span className="text-sm font-semibold text-foreground">{q}</span>
                        <span className="text-primary flex-shrink-0 group-open:rotate-180 transition-transform duration-200 text-lg leading-none" aria-hidden>+</span>
                      </summary>
              ...
              <div>
                <div id="service-form" className="bg-card border border-border/60 p-7 lg:sticky lg:top-24">
                  {status === "success" ? (
                    ...
                    <>
                      ...
                      <form name={`${p.title.toLowerCase().replace(/\s+/g, "-")}-inquiry`}
                        ...
                        {[
                          { id: "name",  label: "Your Name", type: "text",  required: true,  placeholder: "Jane Smith" },
                          { id: "phone", label: "Phone",     type: "tel",   required: false, placeholder: "(541) 555-0100" },
                          { id: "email", label: "Email",     type: "email", required: true,  placeholder: "jane@email.com" },
                        ].map(f => (
                          <div key={f.id}>
                            <label htmlFor={f.id}
                                   className="block text-[10px] tracking-[0.2em] uppercase text-muted-foreground/70 mb-2 font-medium"
                                   style={{ fontFamily: "var(--font-condensed)" }}>
                              {f.label}{f.required && <span className="text-primary ml-1" aria-hidden>*</span>}
                            </label>
                            <input id={f.id} name={f.id} type={f.type}
                                   required={f.required} autoComplete={f.id}
                        ...
                        {status === "error" && (
                          <p className="text-sm text-destructive" role="alert">
                            Error — call us at <a href={SITE.phoneHref} className="underline">{SITE.phone}</a>
                          </p>
                        )}
                        <button type="submit" disabled={status === "submitting"}
                                ...
                                style={{ fontFamily: "var(--font-condensed)" }}>
                          {status === "submitting" ? "Sending…" : <>{`Request ${p.title} Estimate`} <ArrowRight className="h-3.5 w-3.5" /></>}
                        </button>
                        <a href={SITE.phoneHref}
