...
export default function PortalFinishes() {
  ...
  const approveMut = trpc.finishSelections.clientApprove.useMutation({
    onSuccess: () => { utils.finishSelections.list.invalidate(); toast.success("Selection approved"); },
  });

  const fmt = (n: number | string | null | undefined) => n ? `$${Number(n).toLocaleString()}` : "—";
  const totalDelta = selections?.reduce((sum: number, s: any) => sum + (Number(s.budget_delta) || 0), 0) ?? 0;

  // Group by room
  const grouped = (selections ?? []).reduce((acc: Record<string, any[]>, s: any) => {
        ...
        <div className="container py-10 max-w-3xl">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            {selections && selections.length > 0 && (
              <div className={`inline-flex items-center gap-2 px-4 py-2 mb-8 border ${
                totalDelta > 0 ? "border-red-400/30 bg-red-400/5 text-red-400"
                : totalDelta < 0 ? "border-green-400/30 bg-green-400/5 text-green-400"
                : "border-border/60 bg-card text-muted-foreground"
              }`}>
                <DollarSign className="h-3.5 w-3.5" />
                <span className="text-sm font-semibold">
                  Net budget impact: {totalDelta > 0 ? "+" : ""}{fmt(totalDelta)}
                </span>
              </div>
            )}
          </motion.div>
          ...
          {isLoading ? (
            <div className="bg-card border border-border/60 p-12 text-center text-muted-foreground text-sm">Loading selections…</div>
          ) : !selections?.length ? (
            <div className="bg-card border border-border/60 p-12 text-center">
              ...
              <p className="text-muted-foreground text-sm">No finish selections yet. Eric will add options as your project progresses.</p>
            </div>
          ) : (
              ...
              {Object.entries(grouped).map(([room, items]) => (
                <div key={room}>
                  <p className="text-[10px] font-bold tracking-[0.18em] uppercase text-muted-foreground mb-3"
                     style={{ fontFamily: "var(--font-condensed)" }}>{room}</p>
                  <div className="space-y-2">
                    {(items as any[]).map((sel: any) => (
                      <div key={sel.id} className="bg-card border border-border/60 p-4 flex items-start gap-4">
                        {/* Swatch / image */}
                        {sel.image_url ? (
                          <img src={sel.image_url} alt={sel.item_name} className="h-16 w-16 object-cover border border-border/40 shrink-0" />
                        ) : (
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {[sel.brand, sel.color_name, sel.sku ? `SKU: ${sel.sku}` : null].filter(Boolean).join(" · ")}
                              </p>
                            </div>
                            <div className="text-right shrink-0">
                              <p className="text-sm font-semibold">{fmt(sel.total_cost)}</p>
                              {sel.budget_delta != null && Number(sel.budget_delta) !== 0 && (
                                <p className={`text-xs font-bold ${Number(sel.budget_delta) > 0 ? "text-red-400" : "text-green-400"}`}>
                                  {Number(sel.budget_delta) > 0 ? "+" : ""}{fmt(sel.budget_delta)}
                                </p>
                              )}
                            </div>
                          </div>
                          {sel.notes && <p className="text-xs text-muted-foreground/70 mt-1 font-light">{sel.notes}</p>}

                          {/* Approval */}
                          <div className="mt-3 flex items-center gap-3">
                            {sel.client_approved ? (
                              <span className="flex items-center gap-1 text-[10px] font-bold tracking-widest uppercase text-green-400"
                                    style={{ fontFamily: "var(--font-condensed)" }}>
                                <Check className="h-3 w-3" /> Approved {sel.client_approved_at ? new Date(sel.client_approved_at).toLocaleDateString() : ""}
                              </span>
                            ) : (
                              <button onClick={() => approveMut.mutate({ id: sel.id })} disabled={approveMut.isPending}
                            ...
                            {sel.eric_approved && (
                              <span className="text-[10px] text-muted-foreground/40 flex items-center gap-1">
                                <Check className="h-2.5 w-2.5" /> Confirmed by Eric
                              </span>
                            )}
                          </div>
                        </div>
