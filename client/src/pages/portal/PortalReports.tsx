...
import { ArrowLeft, BookOpen, Calendar, CheckCircle2, LogOut } from "lucide-react";
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="pt-[64px]">
        <div className="container py-10 max-w-3xl">
          ...
          {isLoading ? (
            <div className="bg-card border border-border/60 p-12 text-center text-muted-foreground text-sm">Loading reports…</div>
          ) : !reports?.length ? (
            <div className="bg-card border border-border/60 p-12 text-center">
              ...
              <p className="text-muted-foreground text-sm">No reports published yet. Eric will share updates as work progresses.</p>
            </div>
          ) : (
              ...
              {reports.map((report: any) => {
                ...
                return (
                  <motion.div key={report.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <p className="text-sm text-foreground mb-4 leading-relaxed">{report.summary}</p>
                    {tasks.length > 0 && (
                      <div>
                        <p className="text-[10px] font-bold tracking-[0.18em] uppercase text-muted-foreground mb-2" style={{ fontFamily: "var(--font-condensed)" }}>
                          Work Completed
                        </p>
