...
import { getGuideById, type Guide, type GuideSection } from "../pages/admin/guides-data";

function SectionBlock({ section }: { section: GuideSection }) {
  return (
    <div className="space-y-2.5">
      {section.body && (
        <p className="text-xs text-foreground/75 leading-relaxed">{section.body}</p>
      )}
      {section.steps && (
        <div className="space-y-0.5">
          {section.steps.map((step, i) => (
            <div key={i} className="flex gap-2 py-1.5 px-2 rounded hover:bg-muted/30">
        ...
        <div className="bg-amber-500/5 border border-amber-500/15 rounded p-2.5 space-y-1.5">
          ...
          {section.tips.map((tip, i) => (
            <div key={i} className="flex gap-1.5 items-start">
              ...
              <p className="text-[11px] text-foreground/65 leading-relaxed">{tip}</p>
            </div>
          ))}
        </div>
      )}
      {section.warning && (
        <div className="bg-red-500/5 border border-red-500/15 rounded p-2.5 flex gap-2 items-start">
          ...
          <p className="text-[11px] text-red-400/80 leading-relaxed font-medium">{section.warning}</p>
        </div>
      )}
    </div>
  );
        ...
        <SheetHeader className="pb-4 border-b border-border/40">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{guide.tagline}</p>
            </div>
          </div>
        </SheetHeader>

        <div className="py-4">
          <Accordion type="multiple" defaultValue={guide.sections.map((_, i) => `sh-${i}`)}>
