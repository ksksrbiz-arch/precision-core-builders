...
export const handler: Handler = async event => {
  ...
  const days = Math.min(
    Math.max(parseInt(event.queryStringParameters?.days ?? "30", 10) || 30, 1),
    90
  );
  ...
  const byProvider = new Map<string, { calls: number; totalTokens: number }>();
  ...
  const toSorted = (map: Map<string, { calls: number; totalTokens: number }>) =>
