...
const EntryTypeEnum = z.enum(["decision","change_order","inspection","permit","milestone","cost_adjustment","note"]);
...
export const ledgerRouter = router({
  ...
  list: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      const { data, error, count } = await db.from("ledger_entries")
        .select("*", { count: "exact" })
  ...
  listVisible: protectedProcedure
    ...
    .query(async ({ input }) => {
      const { data, error } = await db.from("ledger_entries")
        .select("id,entry_type,title,description,amount_delta,document_url,document_name,created_at")
        .eq("project_id", input.projectId)
  ...
  append: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const { data, error } = await db.from("ledger_entries").insert({
