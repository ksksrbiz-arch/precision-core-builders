...
const TaskTypeEnum = z.enum(["outdoor","indoor","framing","roofing","electrical","plumbing","insulation","drywall","flooring","cabinetry","painting","finish_work","inspection","other"]);
const TaskStatusEnum = z.enum(["pending","in_progress","complete","blocked","deferred"]);
...
export const scheduleRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      const { data, error } = await db.from("schedule_items")
        ...
        .order("sort_order").order("planned_start");
  ...
  update: adminProcedure
    .input(z.object({ id: z.number().int().positive() }).merge(ScheduleItemInput.partial().omit({ projectId: true })))
    .mutation(async ({ input }) => {
      const { id, taskType, isOutdoor, weatherSensitive, plannedStart, plannedEnd,
              durationDays, dependsOn, sortOrder, assignedTo, parentId, ...rest } = input;
      const { data, error } = await db.from("schedule_items").update({
        ...
        ...(weatherSensitive !== undefined && { weather_sensitive: weatherSensitive }),
  ...
  delete: adminProcedure
    ...
    .mutation(async ({ input }) => {
      const { error } = await db.from("schedule_items").delete().eq("id", input.id);
      if (error) throw new Error(error.message);
      return { success: true };
    }),
