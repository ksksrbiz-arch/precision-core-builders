...
function validatePayload(input: NotificationPayload): NotificationPayload {
  ...
  if (title.length > TITLE_MAX_LENGTH)
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: `Title must be ≤ ${TITLE_MAX_LENGTH} characters.`,
    });
  if (content.length > CONTENT_MAX_LENGTH)
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: `Content must be ≤ ${CONTENT_MAX_LENGTH} characters.`,
    });
...
export async function notifyOwner(
  ...
  if (!ENV.n8nWebhookUrl) {
    ...
    console.warn(
      "[Notification] N8N_WEBHOOK_URL not set. Notification dropped:",
      title
    );
    return false;
  }
  ...
  try {
    const res = await fetch(
      `${ENV.n8nWebhookUrl.replace(/\/$/, "")}/owner-notify`,
      {
        method: "POST",
